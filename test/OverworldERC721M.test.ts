...
import { ERC721MOperatorFiltererRoyaltySplitter, PaymentSplitterInitializable } from '../typechain-types';
...
describe('Overworld: ERC721MOperatorFiltererRoyaltySplitter', function () {
  let contract: ERC721MOperatorFiltererRoyaltySplitter;
  let readonlyContract: ERC721MOperatorFiltererRoyaltySplitter;
  let owner: SignerWithAddress;
  let creator: SignerWithAddress;
  let splitterReference: PaymentSplitterInitializable;
  let readonly: SignerWithAddress;
  let chainId: number;

  const getCosignSignature = async (
    contractInstance: ERC721MOperatorFiltererRoyaltySplitter,
  ...
  beforeEach(async () => {
    const PaymentSplitterInitializable = await ethers.getContractFactory('PaymentSplitterInitializable');
    const paymentSplitter = await PaymentSplitterInitializable.deploy();
    await paymentSplitter.deployed();
    const paymentSplitterAddress = paymentSplitter.address;
    splitterReference = paymentSplitter;

    [owner, readonly, creator] = await ethers.getSigners();

    const ERC721MOperatorFiltererRoyaltySplitter = await ethers.getContractFactory('ERC721MOperatorFiltererRoyaltySplitter');
    ...
    const details = {
      ...
      timestampExpirySeconds: 60,
      royaltyFeeNumerator: 1000,
      minterShares: 25,
      creatorShares: 75,
      creator: creator.address,
      paymentSplitterReference: paymentSplitterAddress,
    }
    const eRC721MOperatorFiltererRoyaltySplitter = await ERC721MOperatorFiltererRoyaltySplitter.deploy(
      details
    );
  ...
  describe('Minting', function () {
    it('revert if contract is not mintable', async () => {
      ...
      const setActiveStage = contract.getActiveStageFromTimestamp(0);
      await expect(setActiveStage).to.not.be.revertedWith('InvalidStage');
    ...
    it('crossmint', async () => {
      ...
      const ERC721MOperatorFiltererRoyaltySplitter = await ethers.getContractFactory('ERC721MOperatorFiltererRoyaltySplitter');
      const details = {
        collectionName: "Howaboutanothername",
        collectionSymbol: "TEST",
        tokenURISuffix: ".json",
        maxMintableSupply: 1000,
        ...
        creatorShares: 75,
        creator: creator.address,
        paymentSplitterReference: splitterReference.address,
      }
      const eRC721MOperatorFiltererRoyaltySplitter = await ERC721MOperatorFiltererRoyaltySplitter.deploy(
        details
      );
      ...
      const ownerConn = eRC721MOperatorFiltererRoyaltySplitter.connect(owner);
      const block = await ethers.provider.getBlock(
        await ethers.provider.getBlockNumber(),
      );
      // +10 is a number bigger than the count of transactions up to mint
      ...
      const crossMintConn = eRC721MOperatorFiltererRoyaltySplitter.connect(crossmintSigner);
    ...
    it('crossmint with cosign', async () => {
      ...
      const signature = await getCosignSignature(
        eRC721MOperatorFiltererRoyaltySplitter,
        owner,
  ...
  describe("Royalties and Splits", () => {
    beforeEach(async ()=> {
      //set a mintable stage
      await contract.setStages([
        {
          price: ethers.utils.parseEther('0.5'),
          walletLimit: 1,
    ...
    it('mint and updates royalties', async () => {
      const block = await ethers.provider.getBlock(
        await ethers.provider.getBlockNumber(),
      );
      // +10 is a number bigger than the count of transactions up to mint
      ...
      const ownerSplitters = await contract.paymentSplittersOfMinter(owner.address)
      expect(ownerSplitters).to.not.be.undefined;
      expect(ownerSplitters.length).to.be.equal(100);
    });

    it('mints by owner and updates royalties', async () => {
      const [owner, address1] = await ethers.getSigners();
      ...
      expect((await contract.paymentSplittersOfMinter(address1.address)).length).to.be.equal(0);
      ...
      const ownerSplitters = await contract.paymentSplittersOfMinter(owner.address)
      expect(ownerSplitters).to.not.be.undefined;
      expect(ownerSplitters.length).to.be.equal(ownerBalance.toNumber());
    ...
    it("resets recipient to address zero after burns", async () => {
      const [minter, secondaryOwner] = await ethers.getSigners();
      const tokenId = 0; // Specify the token ID
      const salePrice = 100; // Specify the sale price
  
      // Mint the token
      const tokenMintResp = await contract.ownerMint(1, minter.address);
      // console.log(tokenMintResp);
      
      // Transfer the token from minter to secondary owner
      await contract.transferFrom(minter.address, secondaryOwner.address, tokenId);
  
      // Burn the token
      await contract.burn(tokenId);
  
      // Get the royalty information after the burn
      const [ recipient, value ] = await contract.royaltyInfo(tokenId, salePrice);
  
      expect(recipient).to.equal("0x0000000000000000000000000000000000000000");
      console.log(value);
      expect(value).to.equal((salePrice * 1000 / 10000));
    });


    it("reverts when splitter reference is address zero", async () => {
      const ERC721MOperatorFiltererRoyaltySplitter = await ethers.getContractFactory('ERC721MOperatorFiltererRoyaltySplitter');
      ...
      const details = {
        ...
        creatorShares: 75,
        creator: creator.address,
        paymentSplitterReference: ethers.constants.AddressZero, //bad address
      }
      const eRC721MOperatorFiltererRoyaltySplitter = ERC721MOperatorFiltererRoyaltySplitter.deploy(
        details
      );
    ...
    it("reverts when minter shares are zero", async () => {
      const ERC721MOperatorFiltererRoyaltySplitter = await ethers.getContractFactory('ERC721MOperatorFiltererRoyaltySplitter');
      ...
      const details = {
        ...
        timestampExpirySeconds: 60,
        royaltyFeeNumerator: 1000,
        minterShares: 0, //bad shares
        creatorShares: 75,
        creator: creator.address,
        paymentSplitterReference: splitterReference.address, //bad address
      }
      const eRC721MOperatorFiltererRoyaltySplitter = ERC721MOperatorFiltererRoyaltySplitter.deploy(
        details
      );
    ...
    it("reverts when creator shares are zero", async () => {
      const ERC721MOperatorFiltererRoyaltySplitter = await ethers.getContractFactory('ERC721MOperatorFiltererRoyaltySplitter');
    ...
    it("royalty info for unminted token IDs", async () => {
      const tokenId = 0; // Specify the token ID
      const salePrice = 100; // Specify the sale price
    ...
    // IGNORE: not fully translated yet from solidity
    // it("checks royalty info for safe minted token IDs", async () => {
    //   const [minter] = await ethers.getSigners(); // Specify the minter's address
    //   const tokenId = 0; // Specify the token ID
    //   const salePrice = 100; // Specify the sale price
    
    //   // Mint the token using safe mint
    //   await contract.safeMint(minter.address, 
    //     1
    //   );
    
    //   const paymentSplitterOfToken = await contract.paymentSplitterOf(tokenId);
    //   const paymentSplittersOfMinter = await contract.paymentSplittersOfMinter(minter.address);
    
    //   // Assert that the minter and defaultTokenCreator are not equal to paymentSplitterOfToken
    //   expect(minter.address).to.not.equal(paymentSplitterOfToken);
    //   expect(defaultTokenCreator).to.not.equal(paymentSplitterOfToken);
    
    //   // Perform some transactions and assertions
    //   await contract.deal(paymentSplitterOfToken, ethers.utils.parseEther('1'));
    //   await coinMock.mint(paymentSplitterOfToken, ethers.utils.parseEther('1'));
    
    //   expect(await contract.minterOf(tokenId)).to.equal(minter.address);
    //   expect(paymentSplittersOfMinter.length).to.equal(1);
    //   expect(paymentSplittersOfMinter[0]).to.equal(paymentSplitterOfToken);
    
    //   const [recipient, value] = await contract.royaltyInfo(tokenId, salePrice);
    //   expect(recipient).to.equal(paymentSplitterOfToken);
    //   expect(value).to.equal((salePrice.mul(await contract.royaltyFeeNumerator())).div(await contract.FEE_DENOMINATOR()));
    
    //   const splitter = await ethers.getContractAt('PaymentSplitterInitializable', paymentSplitterOfToken);
    
    //   const minterShares = await splitter.shares(minter.address);
    //   const creatorShares = await splitter.shares(defaultTokenCreator);
    
    //   expect(minterShares).to.equal(25);
    //   expect(creatorShares).to.equal(75);
    
    //   expect(await contract.releasableNativeFunds(tokenId, 0)).to.equal(ethers.utils.parseEther('0.25'));
    //   expect(await contract.releasableNativeFunds(tokenId, 1)).to.equal(ethers.utils.parseEther('0.75'));
    //   expect(await contract.releasableERC20Funds(tokenId, coinMock.address, 0)).to.equal(ethers.utils.parseEther('0.25'));
    //   expect(await contract.releasableERC20Funds(tokenId, coinMock.address, 1)).to.equal(ethers.utils.parseEther('0.75'));
    
    //   await contract.releaseNativeFunds(tokenId, 0);
    //   expect(await ethers.provider.getBalance(minter.address)).to.equal(ethers.utils.parseEther('0.25'));
    
    //   await contract.releaseNativeFunds(tokenId, 1);
    //   expect(await ethers.provider.getBalance(defaultTokenCreator)).to.equal(ethers.utils.parseEther('0.75'));
    
    //   await contract.releaseERC20Funds(tokenId, coinMock.address, 0);
    //   expect(await coinMock.balanceOf(minter.address)).to.equal(ethers.utils.parseEther('0.25'));
    
    //   await contract.releaseERC20Funds(tokenId, coinMock.address, 1);
    //   expect(await coinMock.balanceOf(defaultTokenCreator)).to.equal(ethers.utils.parseEther('0.75'));
    // });
  ...
  describe('Global wallet limit', function () {
    it('validates global wallet limit in constructor', async () => {
      ...
      await expect(
        ERC721MOperatorFiltererRoyaltySplitter.deploy(details),
  ...
  describe('Cosign', () => {
    ...
    it('can deploy with cosign', async () => {
      ...
      const minterConn = eRC721MOperatorFiltererRoyaltySplitter.connect(minter);
